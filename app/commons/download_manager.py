...
from app.commons.locks import release_locks, lock_resource
...
class DownloadClient(object):
    ...
    def __init__(self, files, auth_token, operator, status_code, session_id, download_type="project"):
        ...
        self.tmp_folder = ConfigClass.MINIO_TMP_PATH + status_code + '_' + str(time.time())
    ...
    def set_status(self, status, payload={}):
        if len(self.files_to_zip) > 0:
            geid = self.files_to_zip[0]["geid"]
        else:
            geid = self.status_code
        return set_status(
            self.session_id,
            self.job_id,
            self.result_file_name,
    ...
    def add_files_to_list(self, geid):
        ...
        if not resp.json():
            ...
            self.logger.info(f'Got files from folder: {self.files_to_zip}')
        else:
            file = resp.json()[0]
            if not file.get("archived", False):
                self.files_to_zip.append({
                    "location": file["location"],
                    "geid": file["global_entity_id"],
                    "project_code": file.get("project_code", ""),
    ...
    def zip_worker(self, hash_code):
        ...
        try:
            ...
            if self.download_type == "full_dataset":
                self.add_schemas(self.status_code)
